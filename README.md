...
新部署的可选 IPv6 地址和出口接口支持 `!empty` 清空已有默认值，只有明确显示该提示的字段才解释此命令。手动客户端节点向导按实际经过的字段逐项返回，非敏感值作为默认值保留；敏感字段回退后需要重新填写，不会显示原值。

当某个字段本身允许数值 `0`（例如备份保留数量）时，界面会明确说明“0 是有效数值”；此时使用 `/back` 返回。

### 客户端配置：推荐从方案工作台开始

主菜单 `7` → `快速创建方案`：添加节点、确认连接关系，然后生成候选。地区和业务默认值会自动应用，不必逐项填写。

- 工作台可单独修改节点、连接关系、分组顺序与业务默认出口；已填内容不会因为返回工作台而消失。
- 草稿按当前 Windows 用户加密，保存在 `private/client-schemes`；重新打开必须重新校验，不能直接跨账号解密。
- 生成候选不改正式配置；发布需要两端核心校验通过、确认目标及备份，来源或目标变化时拒绝沿用旧确认。
- 高级现有 DNS/TUN/规则通过“导入现有配置”保留；当前不是任意字段的可视化编辑器。
- 旧版高级生成流程仍作为显式兼容入口保留，不再是默认流程。

操作与限制详见 [方案工作台与事务恢复](docs/WORKBENCH-AND-RECOVERY.zh-CN.md)。
