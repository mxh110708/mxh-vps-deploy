...
VPS 私有归档的目录、配置、日志、Token 文件和其他本地文件沿用用户所选目录的权限。唯一例外是实际交给 Windows OpenSSH 使用的受管私钥文件：OpenSSH 会拒绝其他账户可读的私钥，因此工具只对该单个文件设置当前用户独占 ACL，不递归修改目录、不检查或收紧其他文件，也不建立额外运行副本。秘密扫描、Git 忽略和禁止控制台输出秘密仍是强制边界。
...
5. 初始为 22/低位端口时，SSH 暂时同时监听初始、主高位、救援高位端口；服务商已提供非特权高位端口时，复用它作为主端口并只新增救援端口；
6. 从全新连接验证 root/admin 在最终主、救援端口均可用；
7. 防火墙只放行当前过渡所需的唯一 SSH 端口集合（前者三个、后者两个）；
8. Xray、时间、网络和可选 Komari 全部验收；
9. 最后移除未复用的初始端口；若初始高位端口已复用则保留，再次验证主、救援入口。
...
Xray 生成的秘密通过捕获的机器可读标记返回核心；普通控制台和普通日志不打印敏感 stdout。保存普通本地私有文件后不额外改变所在目录的权限模型；SSH 受管私钥仅应用上一节说明的 OpenSSH 最低文件 ACL。
...
候选必须从目标 VPS 实测，至少满足：TCP/443、证书有效、TLS 1.3、ALPN h2、普通 HTTPS 行为、同地区或邻近、20 次采样中扣除 DNS 的 TCP 建连中位通常不超过 15 ms、没有明显大型多租户共享 CDN 特征。完整 TLS 建连时间包含服务端响应和证书握手，仅独立记录作参考，不与 15 ms 网络门槛混用。大学、教育/科研机构、成熟企业或专业机构优先，不选个人站点和教程中被反复复制的热门 target。
...
Padding scheme 不是认证秘密。新计划为每台实例生成一组稳定的保守方案，避免所有部署长期共享同一组示例参数；方案保存在实例计划中，不能在每次重启或继续未完成部署时轮换。客户端第一次建立会话仍使用协议默认方案，之后由服务端在加密协议内下发实例方案，因此它不能消除所有初始连接或时序特征，也不能视为绝对抗识别保证。
...
新部署另有统一事务基线：恢复公钥验证后立即建立，保留该公钥作为紧急入口，再覆盖项目会修改的受管路径、账户状态、systemd 状态、nftables 和 sysctl。放弃未完成部署时严格按“远端恢复 → SSH 复验 → 删除精确快照 → 本地清理”排序；任一步失败都不提前删除后续恢复材料。快照删除只接受固定根目录下的 32 位事务 ID 或时间戳协议备份，并在回滚标记存在时才清理。本次事务新增的服务器时间戳备份会自动删除，基线前已有备份不会触碰。

回滚不自动卸载 apt 新增包，避免依赖级联删除破坏服务商镜像；它会报告保留包数量，并恢复项目受管服务、配置和账户。放弃本地计划时保留 SSH 密钥及用户自行维护的外部 Token 文件，只删除本次计划、状态、秘密和生成的服务端/客户端产物；`abandoned-transactions` 仅保存时间、类型和 SHA-256，不保存秘密值。
