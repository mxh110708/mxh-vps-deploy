...
- 修复受控更新后的进程一致性：显式重启目标代理，并核对 MainPID 使用的二进制；停用备用协议在变更前提示先启用或切换。
- 自动回滚后按已记录恢复点同步还原本地计划、凭据与配置，未处理记录阻止开始新维护。
- 完整恢复拒绝 SSH 端口已经变更的旧防火墙快照，并在恢复后复验管理端口；仅配置恢复增加 nginx 检查与重新加载。
- 健康审计尊重纳管时明确选择的 SSH 认证保留策略，仍把公钥认证关闭视为严重异常。

- 主菜单、协议管理和运维子菜单改用直接描述用途的名称，技术说明移入帮助；保留编号、默认项、确认短语和执行分支。已有 VPS 的记录展示不再误报“放行两个新的 SSH 端口”的新部署提醒。

- 修复已部署实例的受管 SSH 私钥权限漂移：每次发送远端命令前先用 OpenSSH 检查，只在权限过宽时修复该密钥的继承和显式非受信任授权；不修改服务商原始私钥，不重放远端操作，非 ACL 错误不触发权限修改。

- 客户端设计默认改为方案工作台：节点/连接关系局部编辑、Windows 当前用户加密草稿、来源指纹检测、候选生成与发布分离；旧长向导保留为显式兼容入口。
- 双文件发布增加目标锁、唯一备份、确认后变更检测、候选指纹和阶段记录，支持检查并恢复未完成发布；遇到外部修改拒绝自动覆盖。
- 协议与 SSH 维护引入共享远端事务所有者，提交/回滚绑定恢复点；增加遗留事务检查、准备锁受限解除及 SSH 中断记录和只读新旧入口探测。
- 手动恢复按范围合并元数据，拒绝不兼容的仅配置恢复，先准备本地候选再操作远端，本地阶段完成后才撤销恢复保护。

- 修复运维、协议启停/卸载、恢复中心和客户端默认值编辑的多层返回边界；手动节点与网络参数支持按已访问字段回退。
- SSH/代理凭据轮换增加默认拒绝确认，Komari 卸载在确认后启动事务，防火墙接管先备份旧计划再修改模式。
- JSON 改为同目录临时文件替换，准备失败保留旧文件；既有私有归档继续沿用目录权限。菜单校验默认值并统一空白输入语义，合法数值 0 的字段支持 `/back`。
- 增加独立交互输入队列回归测试并接入完整测试入口；详见 [交互维护与回归约定](docs/INTERACTION-MAINTENANCE.md)。
